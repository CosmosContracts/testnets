...
```bash
junod init <moniker-name> --chain-id=scottie-2
```
...
```bash
junod gentx <key-name> 9000000000ujunox --chain-id=scottie-2
```
...
- Copy the generated gentx json file to `<repo_path>/scottie-2/gentx/`

  ```sh
  > cd testnets
  > cp "${HOME}/.juno/config/gentx/gentx*.json" ./scottie-2/gentx/
  ```
...
[Genesis File](/scottie-2/genesis.json):

```bash
   curl -s  https://raw.githubusercontent.com/CosmosContracts/testnets/main/scottie-2/genesis.json > ~/.juno/config/genesis.json
```
