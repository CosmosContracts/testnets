# Junø - astarte-3 Testnet

The plan for this testnet is to simulate upgrading from juno `v2.0.6` to a patch, `v2.1.0`
...
6. Check genesis bin is `v2.0.6`: `$DAEMON_HOME/cosmovisor/genesis/bin/junod version`
7. Follow generate gentx as normal below

**Genesis File**

[Genesis File](/astarte-3/genesis.json):

```bash
   curl -s  https://raw.githubusercontent.com/CosmosContracts/testnets/main/astarte-3/genesis.json > ~/.juno/config/genesis.json
```
...
1. Initialize the Juno directories and create the local genesis file with the correct chain-id:
   ...
   ```bash
   junod init <moniker-name> --chain-id=astarte-3
   ```
...
4. Create the gentx, use only `9000000000ujunox`:
   ...
   ```bash
   junod gentx <key-name> 9000000000ujunox --chain-id=astarte-3
   ```
...
- Copy the generated gentx json file to `<repo_path>/astarte-3/gentx/`

  ```sh
  > cd testnets
  > cp ~/.juno/config/gentx/gentx*.json ./astarte-3/gentx/
  ```
