...
[Genesis File](/tinia/genesis.json):

```bash
   curl -s  https://raw.githubusercontent.com/CosmosContracts/testnets/main/tinia/genesis.json >~/.juno/config/genesis.json
```
...
[Full seed nodes list](/tinia/seeds.txt).
...
1. Initialize the Juno directories and create the local genesis file with the correct chain-id:
   ...
   ```bash
   junod init <moniker-name> --chain-id=tinia
   ```
...
4. Create the gentx, use only `9000000000ujunox`:
   ...
   ```bash
   junod gentx <key-name> 9000000000ujunox --chain-id=tinia
   ```
...
- Copy the generated gentx json file to `<repo_path>/tinia/gentx/`

  ```sh
  > cd testnets
  > cp ~/.juno/config/gentx/gentx*.json ./tinia/gentx/
  ```
