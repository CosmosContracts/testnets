...
[Genesis File](/uni-1/genesis.json):

```bash
   curl -s  https://raw.githubusercontent.com/CosmosContracts/testnets/main/uni-1/genesis.json > ~/.juno/config/genesis.json
```
...
```bash
junod init <moniker-name> --chain-id=uni-1
```
...
```bash
junod gentx <key-name> 9000000000ujunox --chain-id=uni-1
```
...
- Copy the generated gentx json file to `<repo_path>/uni-1/gentx/`

  ```sh
  > cd testnets
  > cp ~/.juno/config/gentx/gentx*.json ./uni-1/gentx/
  ```
